...
def test_cadastrar_aluno_armazena_hash(ctx):
    aluno, erro, codigo = aluno_service.cadastrar_aluno('Ana', 'ana@teste.com', 'ana', 'senha123')
...
def test_cadastrar_aluno_usuario_duplicado(ctx):
    db.session.add(Aluno(nome_completo='A', email='a@a.com', usuario='dup', senha='x'))
    db.session.commit()

    aluno, erro, codigo = aluno_service.cadastrar_aluno('B', 'b@b.com', 'dup', 'y')
    assert aluno is None and erro == 'Usuário já existe' and codigo == 409


def test_cadastrar_aluno_email_duplicado(ctx):
    db.session.add(Aluno(nome_completo='A', email='a@a.com', usuario='um', senha='x'))
    db.session.commit()

    aluno, erro, codigo = aluno_service.cadastrar_aluno('B', 'a@a.com', 'dois', 'y')
    assert aluno is None and erro == 'Email já cadastrado' and codigo == 409


def test_obter_dados_aluno(ctx):
    aluno = Aluno(nome_completo='Ana', email='ana@teste.com', usuario='ana', senha='hash')
